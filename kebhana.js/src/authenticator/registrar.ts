import { createHash, createHmac, generateKeyPairSync, randomBytes, randomUUID } from "crypto";
import { WizveraCrypto } from "../wizvera/wizveracrypto.js";
import axios from "axios";
import jws from "jws";

export class KebHanaPINRegistrar {
    private pinSignURL = "https://mbp.hanabank.com/wizvera/pinsign/svc/pinsign_service.jsp";
    ...
    private crypto1: WizveraCrypto;
    private crypto2: WizveraCrypto;

    private _ecPublic: Buffer;
    private _ecPrivate: string;
    private _jwk: { kty: string, crv: string, x: string, y: string };

    constructor(private pin: string, private _cookie: string, private uuid: string = randomUUID()) {
        ...
        if (!/^[0-9]{6}$/.test(pin)) throw new Error("Invalid PIN format. Please provide a 6-digit PIN.");
    ...
    async register() {
        ...
        if (preprocessing.headerData.status !== 200) throw new Error(`Failed to preprocess PIN registration: ${preprocessing.headerData.errorMessage} (${preprocessing.headerData.errorCode})`);
        ...
        if (pinConfirm.headerData.status !== 200) throw new Error(`Failed to confirm PIN: ${pinConfirm.headerData.errorMessage} (${pinConfirm.headerData.errorCode})`);
        ...
        if (resetPin.headerData.status !== 200) throw new Error(`Failed to reset PIN: ${resetPin.headerData.errorMessage} (${resetPin.headerData.errorCode})`);
        ...
        if (issueNew.resultCode !== 0) throw new Error(`Failed to issue new PIN: ${issueNew.errMsg}`);
        ...
        if (anonym.headerData.status !== 200) throw new Error(`Failed to retrive HWID: ${anonym.headerData.errorMessage} (${anonym.headerData.errorCode})`);

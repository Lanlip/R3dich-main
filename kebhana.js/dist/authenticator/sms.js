...
export class SMSAuthenticator {
    _secureAuthenticator;
    _sent = false;
    _config = null;
    _verified = false;
    constructor(_secureAuthenticator) {
        this._secureAuthenticator = _secureAuthenticator;
    ...
    async requestSMS(config) {
        ...
        if (response.data.headerData.status !== 200)
            throw new Error(`Failed to request SMS authentication: ${response.data.headerData.errorMessage} (${response.data.headerData.errorCode})`);
        ...
        if (smsResponse.data.headerData.status !== 200)
            throw new Error(`Failed to send SMS authentication: ${smsResponse.data.headerData.errorMessage} (${smsResponse.data.headerData.errorCode})`);
        this._config = config;
        this._sent = true;
    }
    async verify(code) {
        if (!this.sent)
            throw new Error("Please request SMS authentication first.");
        if (!this._config)
        ...
        if (response.data.headerData.status !== 200)
            throw new Error(`Failed to verify SMS authentication: ${response.data.headerData.errorMessage} (${response.data.headerData.errorCode})`);
        this._verified = true;
    }
    createPINRegistrar(pin) {
        if (!this._verified)
            throw new Error("SMS authentication must be verified first.");
        if (!this._secureAuthenticator.cookie)
            throw new Error("SecureSMSAuthenticator instance was not initialized");

...
export class SecureSMSAuthenticator {
    ...
    async initialize() {
        ...
        this._cookie = response.headers["set-cookie"]?.find((cookie) => cookie.startsWith("JSESSIONID"))?.split("=")[1].split(";")[0] || null;
        this._authenticationRequest = response.data.data;
    ...
    async check() {
        ...
        if (response.data.headerData.status !== 200)
            throw new Error(`Failed to check authentication status: ${response.data.headerData.errorMessage} (${response.data.headerData.errorCode})`);
